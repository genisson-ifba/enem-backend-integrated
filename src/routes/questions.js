...
async function questionRoutes(fastify, options) {
  ...
  // Get specific exam details (LIGHTWEIGHT - returns only metadata)
  ...
  // Get specific exam with FULL question details (HEAVY - only use when needed)
  fastify.get('/exams/:year/full', async (request, reply) => {
    try {
      const { year } = request.params
      const examPath = join(__dirname, `../data/public/exams/${year}/details.json`)
      const examData = readFileSync(examPath, 'utf-8')
        ...
        try {
          ...
          if (fullQuestion.files) {
            fullQuestion.files = fullQuestion.files.map(file => 
              file.replace(/https:\/\/enem\.dev\/(.+)/, 'https://enem-frontend-integrated.vercel.app/exams/$1')
            )
          }
          ...
          if (fullQuestion.alternatives) {
            fullQuestion.alternatives = fullQuestion.alternatives.map(alt => ({
              ...alt,
              file: alt.file ? alt.file.replace(/https:\/\/enem\.dev\/(.+)/, 'https://enem-frontend-integrated.vercel.app/exams/$1') : alt.file
            }))
          }
